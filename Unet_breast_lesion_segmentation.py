...
# Function to find the best threshold for each mask
def find_best_threshold_per_mask(y_true, y_pred):
    best_thresholds = []
    best_scores = []
    for i in range(len(y_true)):
        ...
        for threshold in np.arange(0.01, 1.0, 0.01):
            thresholded_pred = (y_pred[i] > threshold).astype('float32')
            score = iou_metric(y_true[i], thresholded_pred).numpy()
            if score > best_score:
                best_score = score
...
x_train, temp_images, y_train, temp_masks, train_labels, temp_labels = train_test_split(
    images, masks, encoded_labels, train_size=0.7, random_state=4, stratify=encoded_labels)
x_val, x_test, y_val, y_test, val_labels, test_labels = train_test_split(
    temp_images, temp_masks, temp_labels, test_size=0.5, random_state=4, stratify=temp_labels)
...
# Find the best threshold for each mask
best_thresholds, best_scores = find_best_threshold_per_mask(y_test, predicted_masks)

# Apply the best threshold for each mask
thresholded_masks = [(predicted_masks[i] > best_thresholds[i]).astype('float32') for i in range(len(predicted_masks))]

# Calculate evaluation metrics for each mask
...
for i in range(len(x_test)):
    ...
    if dice > max_dice:
        max_dice = dice
    print(f"Image {i+1}: Best Threshold = {best_thresholds[i]}, IoU = {iou:.4f}, Dice = {dice:.4f}")
...
for i in range(5):
    ...
    iou_score = iou_metric(mask, processed_mask).numpy()
    ...
    plt.title(f"Predicted Mask (IoU: {iou_score:.4f})", fontsize=16)
